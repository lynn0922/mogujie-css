        ...
        <c-header fixed>
            <c-box-skin type="bottom">
                <c-columns pv6 pl10 pr20>
                    <c-svg-left @click="goBack" size="40" :colors="['#c0c0c0', '#fff', '#c0c0c0']" mr10></c-svg-left>
                    <c-search class="search" @click="isShow = !isShow" text="连衣裙" radius2></c-search>
                </c-columns>
            </c-box-skin>
        </c-header>
                    ...
                    <c-row>
                        <c-col v-for="(item, index) in oneCategory" :key="index" span="8">
                            <router-link :to="'/mall/goods?id=' + item['id'] + '&des=' + item['des']">
                                <c-avatar class="good" :url="item['image']" :text="item['des']" direction="vertical"></c-avatar>
                            </router-link>
                        </c-col>
                    </c-row>
                </c-box-skin>
